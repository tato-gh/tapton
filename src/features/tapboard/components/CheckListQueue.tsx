...
import {
  upsertCardReborns,
  getDictByCardId,
} from '@domains/tapboard/storage/cardReborns';
import {
  getWaitingCards,
  updateNextShowTime,
  getCardsReborned,
...
const CheckListQueue: FC = () => {
  ...
  useLayoutEffect(() => {
    (async () => {
      ...
      setQueue(() => {
        const queueReborns = randomPick(
          cardsReborned,
          cardsReborned.length
        ).map((card, ind) => {
          return Object.assign(card, { no: ind + 1, reborned: true });
        });

        const queueNotifiedCards = randomPick(
          notifiedCards,
          numNotifiedCards
        ).map((card, ind) => {
        ...
        const queueNonNotifiedCards = randomPick(
          nonNotifiedCards,
          numNonNotifiedCards
        ).map((card, ind) => {
          return Object.assign(card, {
            no: ind + 1 + numReborns + numNotifiedCards,
            reborned: false,
          });
        });
      ...
      if (cardsReborn.length != 0) {
        ...
        cardsReborn.forEach((card, ind) => {
          ...
          const queueCard = Object.assign(card, { no, reborned: true });
  ...
  return (
    <CheckList queue={queue} onPress={onPress} onPressSkip={onPressSkip} />
  );
};
